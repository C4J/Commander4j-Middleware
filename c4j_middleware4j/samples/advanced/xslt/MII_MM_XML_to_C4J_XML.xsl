	...
	<xsl:template match="E1MARAM">
		...
		<xsl:if test="$Enable = 'Y'">
			<xsl:if test="$Criteria != ''">
				<debug_desc><xsl:value-of select="$DESCRIPTION"/></debug_desc>
				<debug_criteria><xsl:value-of select="$Criteria"/></debug_criteria>
				<debug_matcheddescriptionEnd><xsl:value-of select="$matcheddescriptionEnd"/></debug_matcheddescriptionEnd>
				
				<xsl:if test="$matcheddescriptionEnd != ''">
					
						<xsl:if test="$Enable_Pack_Override = 'Y'">
							<pack_label_module_id><xsl:value-of select="$Pack_Module_ID"/></pack_label_module_id>
						</xsl:if>
					
						<xsl:if test="$Enable_Pallet_Override = 'Y'">
							<pallet_label_module_id><xsl:value-of select="$Pallet_Module_ID"/></pallet_label_module_id>
						</xsl:if>
						...
						<xsl:if test="$Enable_ALT_TEXT_1 = 'Y'">
							<materialCustomerData>
								<customer>
									...
									<data>
										<xsl:attribute name="dataType"><xsl:value-of select="$ALT_TEXT_1_Data_ID"/></xsl:attribute>
										<xsl:attribute name="value"><xsl:value-of select="$ALT_TEXT_1_Data"/></xsl:attribute>
									</data>					
								</customer>
							</materialCustomerData>
						</xsl:if>
		...
		<materialUOMDefinition>
			...
			<xsl:if test="$current_uom='D97'">
				...
				<xsl:if test="$LE_UOM!=''">
					...
					<xsl:comment>Calculated LE quantity is <xsl:value-of select="$temp99"/>
					</xsl:comment>
