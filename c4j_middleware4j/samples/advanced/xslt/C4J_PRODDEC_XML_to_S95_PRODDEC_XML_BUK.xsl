...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
	...
	<xsl:template match='message'>
		...
		<ProductionPerformance>
			...
			<ProductionResponse>
				<ProductionRequestID>
					<xsl:variable name="PO_SHORT1" select="/message/messageData/productionDeclaration/processOrder"/>							
					<xsl:variable name="PO_SHORT2" select="concat('00000000000000000000',$PO_SHORT1)"/>
					<xsl:variable name="STRLEN" select="string-length($PO_SHORT2)"/>	
					<xsl:variable name="PO_LONG" select="substring($PO_SHORT2,$STRLEN - 12 + 1,12)"/>
					<xsl:value-of select='$PO_LONG'/>
				</ProductionRequestID>
				<SegmentResponse>
					<ID>
					...
					<MaterialProducedActual>
						...
						<globe_CoproductFlag/>
						<globe_MovementType>101</globe_MovementType>
					</MaterialProducedActual>
				</SegmentResponse>
			</ProductionResponse>
			<globe_UserName>
