...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:c4j="http://www.commander4j.com" xmlns:c4j_XSLT_Ext="http://com.commander4j.Transformation" exclude-result-prefixes="xs c4j c4j_XSLT_Ext" version="2.0">
	...
	<xsl:variable name="LOCATION">
		<xsl:value-of select="c4j:getConfigItem('config','Location')"/>
	</xsl:variable>
	<!--  *****************  -->
	<!--  NOT FOR EU CONFIG  -->
	<!--  *****************  -->
	<xsl:variable name="Enable">
		<xsl:value-of select="c4j:getConfigItem('non_eu','Enable')"/>
	</xsl:variable>
	<xsl:variable name="Criteria">
		<xsl:value-of select="c4j:getConfigItem('non_eu','Criteria')"/>
	</xsl:variable>
	<xsl:variable name="Enable_Pack_Override">
		<xsl:value-of select="c4j:getConfigItem('non_eu','Enable_Pack_Override')"/>
	</xsl:variable>
	<xsl:variable name="Pack_Module_ID">
		<xsl:value-of select="c4j:getConfigItem('non_eu','Pack_Module_ID')"/>
	</xsl:variable>
	<xsl:variable name="Enable_Pallet_Override">
		<xsl:value-of select="c4j:getConfigItem('non_eu','Enable_Pallet_Override')"/>
	</xsl:variable>
	<xsl:variable name="Pallet_Module_ID">
		<xsl:value-of select="c4j:getConfigItem('non_eu','Pallet_Module_ID')"/>
	</xsl:variable>
	<xsl:variable name="Enable_ALT_TEXT_1">
		<xsl:value-of select="c4j:getConfigItem('non_eu','Enable_ALT_TEXT_1')"/>
	</xsl:variable>
	<xsl:variable name="ALT_TEXT_1_Cust_ID">
		<xsl:value-of select="c4j:getConfigItem('non_eu','ALT_TEXT_1_Cust_ID')"/>
	</xsl:variable>
	<xsl:variable name="ALT_TEXT_1_Data_ID">
		<xsl:value-of select="c4j:getConfigItem('non_eu','ALT_TEXT_1_Data_ID')"/>
	</xsl:variable>
	<xsl:variable name="ALT_TEXT_1_Data">
		<xsl:value-of select="c4j:getConfigItem('non_eu','ALT_TEXT_1_Data')"/>
	</xsl:variable>
	<!--  
	...
	<xsl:template match="EDI_DC40">
		...
		<messageRef>
			DOCNUM
			<xsl:value-of select="$SAPDOCNUM_SHORT"/>
		</messageRef>
		<messageInformation>
			Material=
		...
		<messageDate>
			<xsl:value-of select="c4j_XSLT_Ext:formatDate($CREATE_DATETIME, 'yyyyMMddHHmmss', 'yyyy-MM-dd''T''HH:mm:ss')"/>
		</messageDate>
	</xsl:template>
	<xsl:template match="E1MARAM">
		...
		<material>
			<xsl:value-of select="$MATERIAL_SHORT"/>
		</material>
		<old_material/>
		<materialType>
			<xsl:value-of select="string(MTART)"/>
		</materialType>
		<base_uom>
			<xsl:value-of select="string(MEINS)"/>
		</base_uom>
		<gross_weight>
		...
		<weight_uom>
			<xsl:value-of select="string(GEWEI)"/>
		</weight_uom>
		<xsl:comment>
		...
		<xsl:variable name="matcheddescriptionEnd" select="string(E1MAKTM/SPRAS[.=$LANGUAGE]/../MAKTX[ends-with(../MAKTX, $Criteria)])"/>
		<description>
		...
		<xsl:if test="$Enable = 'Y'">
			<xsl:if test="$Criteria != ''">
				...
				<debug_matcheddescriptionEnd>
					<xsl:value-of select="$matcheddescriptionEnd"/>
				</debug_matcheddescriptionEnd>
				<xsl:if test="$matcheddescriptionEnd != ''">
					<xsl:if test="$Enable_Pack_Override = 'Y'">
						<pack_label_module_id>
							<xsl:value-of select="$Pack_Module_ID"/>
						</pack_label_module_id>
					</xsl:if>
					<xsl:if test="$Enable_Pallet_Override = 'Y'">
						<pallet_label_module_id>
							<xsl:value-of select="$Pallet_Module_ID"/>
						</pallet_label_module_id>
					</xsl:if>
					<xsl:if test="$Enable_ALT_TEXT_1 = 'Y'">
						<materialCustomerData>
							<customer>
								<xsl:attribute name="ID">
									<xsl:value-of select="$ALT_TEXT_1_Cust_ID"/>
								</xsl:attribute>
								<data>
									<xsl:attribute name="dataType">
										<xsl:value-of select="$ALT_TEXT_1_Data_ID"/>
									</xsl:attribute>
									<xsl:attribute name="value">
										<xsl:value-of select="$ALT_TEXT_1_Data"/>
									</xsl:attribute>
								</data>
							</customer>
						</materialCustomerData>
		...
		<equipment_Type>
			<xsl:value-of select=" /ZMATMAS03/IDOC/E1MARAM/E1MLGNM/LGNUM[.=$WAREHOUSE]/../LETY1"/>
		</equipment_Type>
		<xsl:comment>
			Use PLANT = [
			<xsl:value-of select="$USE_PLANT"/>
			]
		</xsl:comment>
		...
		<xsl:if test="$USE_PLANT = ''">
			<shelf_life>
				<xsl:value-of select="c4j_XSLT_Ext:trim(MHDHB)"/>
			</shelf_life>
			<xsl:variable name="shelf_life_uom" select="c4j_XSLT_Ext:trim(string(IPRKZ))"/>
			<shelf_life_uom>
				<xsl:value-of select="c4j:getReferenceItem('ShelfLifeUom',$shelf_life_uom)"/>
			</shelf_life_uom>
			<!--<shelf_life_rule1><xsl:value-of select="RDMHD" /></shelf_life_rule1>-->
		</xsl:if>
		<shelf_life_rule>
			<xsl:value-of select="RDMHD"/>
		</shelf_life_rule>
	...
	<xsl:template match="E1MARCM/_-GLB_-RGTE1MARCMBBD">
		<shelf_life>
			<xsl:value-of select="c4j_XSLT_Ext:trim(PLANT_MHDHB)"/>
		</shelf_life>
	...
	<xsl:template match="E1MARMM">
		<materialUOMDefinition>
			...
			<uom>
				<xsl:value-of select="$current_uom"/>
			</uom>
			<ean>
				<xsl:value-of select="$current_ean"/>
			</ean>
			<variant>
				<xsl:value-of select="$current_variant"/>
			</variant>
			<xsl:if test="$current_uom='D97'">
				...
				<xsl:if test="$LE_UOM!=''">
					...
					<xsl:comment>
						LE Quantity =
						<xsl:value-of select="$LE_QTY"/>
					</xsl:comment>
					<xsl:comment>
						LE Unit =
						<xsl:value-of select="$LE_UOM"/>
					</xsl:comment>
					<xsl:comment>
						LE Numerator =
						<xsl:value-of select="$LE_NUMERATOR"/>
					</xsl:comment>
					<xsl:comment>
						LE Denominator =
						<xsl:value-of select="$LE_DENOMINATOR"/>
					</xsl:comment>
					<xsl:comment>* BASE VALUES *</xsl:comment>
					<xsl:comment>
						Base Unit =
						<xsl:value-of select="$BASE_UOM"/>
					</xsl:comment>
					<xsl:comment>* CALCULATIONS *</xsl:comment>
					<xsl:comment>
						Converting LE Quantity from
						<xsl:value-of select="$LE_UOM"/>
						to
						<xsl:value-of select="$BASE_UOM"/>
					</xsl:comment>
					...
					<xsl:if test="$temp99=0">
						<numerator>
							<xsl:value-of select="c4j_XSLT_Ext:trim(UMREZ)"/>
						</numerator>
					</xsl:if>
					<xsl:if test="$temp99>0">
						<xsl:if test="$temp98!=$temp99">
							<xsl:comment>
								LE Qty
								<xsl:value-of select="$temp99"/>
								is different to Normal Qty
								<xsl:value-of select="$temp98"/>
				...
				<xsl:if test="$LE_UOM=''">
					...
					<numerator>
						<xsl:value-of select="$current_numerator"/>
					</numerator>
				</xsl:if>
			</xsl:if>
			<xsl:if test="$current_uom!='D97'">
			...
			<denominator>
				<xsl:value-of select="$current_denominator"/>
			</denominator>
		</materialUOMDefinition>
	</xsl:template>
	<xsl:template match="E1MARDM">
		...
		<xsl:variable name="current_locn">
			<xsl:value-of select="c4j:getReferenceItem('PlantSLOCtoLocation',$temp2)"/>
		</xsl:variable>
		<xsl:if test="$current_locn!=''">
			<location>
				<location>
					<xsl:value-of select="$current_locn"/>
				</location>
				<status>
					<xsl:value-of select="c4j:getReferenceItem('SAPMaterialStatus',$current_status)"/>
				</status>
			</location>
		</xsl:if>
	</xsl:template>
