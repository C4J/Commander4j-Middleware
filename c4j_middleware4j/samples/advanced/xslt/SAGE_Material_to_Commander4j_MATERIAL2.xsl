...
<xsl:stylesheet xmlns:p="http://com.sage/X3/WWIMPFHITMCM"
	...
	<xsl:template match="I">
		...
		<xsl:variable name="base_uom_c4j" select="c4j:getReferenceItem('SageUOM', $base_uom)"/>
		
		<xsl:variable name="prod_uom" select="string(prod_uom)"/>
		<xsl:variable name="prod_uom_c4j" select="c4j:getReferenceItem('SageUOM', $prod_uom)"/>
		...
		<message>
			...
			<messageData>
				<materialDefinition>
					...
					<xsl:if test="$base_uom_c4j=''">
						<xsl:comment>Invalid base_uom from SAGE xml</xsl:comment>
						<xsl:comment>Sage [<xsl:value-of select="$base_uom"/>] C4J Lookup [<xsl:value-of select="$base_uom_c4j"/>]</xsl:comment>
					...
					<xsl:if test="$base_uom_c4j!=''">
							...
							<materialUOMDefinition>
								<uom>
									<xsl:value-of select="$base_uom_c4j"/>
								</uom>
							...
							<xsl:if test="$prod_uom_c4j=''">
								<xsl:comment>Invalid prod_uom from SAGE xml</xsl:comment>
								<xsl:comment>Sage [<xsl:value-of select="$prod_uom"/>] C4J Lookup [<xsl:value-of select="$prod_uom_c4j"/>]</xsl:comment>
							...
							<xsl:if test="$prod_uom_c4j!=''">
								...
								<materialUOMDefinition>
									<uom>
										<xsl:value-of select="$prod_uom_c4j"/>
									</uom>
									<numerator>
